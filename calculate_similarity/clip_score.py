...
model, preprocess = clip.load("/mnt/petrelfs/sunhaoyu/work/clip_model/ViT-B-32.pt", device=device)
...
def safe_open_image(image_path):
    ...
    try:
        ...
        if pixels > max_pixels:
            ...
            print(f"Image {os.path.basename(image_path)} has been resized: {width}x{height} -> {new_width}x{new_height}")
...
def update_category_summary(data, category):
    ...
    """
    category_results = [item for item in data["results"] if item.get("Category") == category and item.get("id") != f"{category}_summary"]
    
    if not category_results:
        return False
    
    # Calculate average scores
    clip_scores = [item.get("clip_score", 0) for item in category_results if isinstance(item.get("clip_score", 0), (int, float))]
...
def compare_model_datasets(source_folder1, source_folder2, output_dir="./results"):
    ...
    # Model list
    models = ["claude", "gemini", "internvl2", "internvl3", "llava", "o1", "openai", "qwen", "o4mini","pro"]
    # Dataset list
    datasets = ["Code Refinement", "Image_to_code", "Interaction_Authoring", "Text_to_code"]
    ...
    for model in models:
        ...
        current_ids = [item.get("id", "") for item in data["results"] if not isinstance(item.get("id"), str) or not item.get("id", "").endswith("_summary")]
        ...
        for dataset in datasets:
            ...
            if not os.path.exists(dataset_folder1) or not os.path.exists(dataset_folder2):
                print(f"Dataset folder {dataset_folder1} or {dataset_folder2} does not exist, skipping")
                continue
            
            # Get image filenames from both folders
            files1 = [f for f in os.listdir(dataset_folder1) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff'))]
            files2 = [f for f in os.listdir(dataset_folder2) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff'))]
            
            # Find common filenames between the two folders
            common_files = list(set(files1) & set(files2))
            
            if not common_files:
                print(f"Warning: No matching image files found in dataset {dataset} folders")
                continue
            
            processing_count = 0
            
            # Calculate similarity for each pair of images
            for filename in tqdm(common_files, desc=f"Processing image similarity for {model}/{dataset}"):
